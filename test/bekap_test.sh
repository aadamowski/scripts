#!/bin/sh
# Unit tests for bekap.sh using shunit2.

SCRIPT="/scripts/bekap.sh"
...
test_backup_existing_file() {
    "$SCRIPT" "$T/file1.txt"
    assertTrue "Backup bz2 file should exist" "ls \"$T/bekap\"/file1.txt.*.bz2 >/dev/null 2>&1"
    assertTrue "Original file should still exist" "[ -f \"$T/file1.txt\" ]"
}

test_backup_multiple_files() {
    "$SCRIPT" "$T/file1.txt" "$T/file2.txt"
    assertTrue "file1 backup exists" "ls \"$T/bekap\"/file1.txt.*.bz2 >/dev/null 2>&1"
    assertTrue "file2 backup exists" "ls \"$T/bekap\"/file2.txt.*.bz2 >/dev/null 2>&1"
}
